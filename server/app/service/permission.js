...
class PermissionService extends Service {
  async index() {
    const sql = 'SELECT id,name,code FROM p_permission WHERE pid = any(SELECT id FROM p_permission WHERE pid=-1)';
    // const sql1 = `UPDATE p_user SET name='${params.name}',gender='${params.gender}',age='${params.age}',phone='${params.phone}' WHERE id=${params.id}`;
